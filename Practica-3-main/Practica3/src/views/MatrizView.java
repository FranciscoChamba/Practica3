...
public class MatrizView extends javax.swing.JDialog {
    ...
    private void botonMatriz() {
        try {
            ...
            controller = new MatrizController(filas, columnas);
            controller.llenarMatrizAleatorio();

            int[][] datos = controller.getDatos();
            Object[][] datosTabla = new Object[filas][columnas];
            for (int i = 0; i < filas; i++) {
                for (int j = 0; j < columnas; j++) {
                    datosTabla[i][j] = datos[i][j];
                }
            }
            // Crear encabezados (nombres de columnas)
            String[] columnasNombres = new String[columnas];
            for (int i = 0; i < columnas; i++) {
                columnasNombres[i] = "Col " + (i + 1);
            }

            // Setear datos y encabezados directamente en la tabla
    ...
    private void actualizarTabla() {
        int[][] matriz = controller.getDatos();

        DefaultTableModel modelo = (DefaultTableModel) tblMatriz.getModel();

        modelo.setRowCount(0);
        modelo.setColumnCount(matriz.length > 0 ? matriz[0].length : 0);
      ...
      private void botonEliminar() {
        ...
        if (numeroStr.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Por favor, ingrese un número en el campo de texto para eliminar.", "Advertencia", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        try {
            ...
            if (eliminado) {
                actualizarTabla(); 
                JOptionPane.showMessageDialog(this, "El número " + numeroAEliminar + " ha eliminado y reemplazado con '*'.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
                txtNumeroaEliminar.setText(""); 
            } else {
                JOptionPane.showMessageDialog(this, "El número " + numeroAEliminar + " no se encontró en la matriz.", "Información", JOptionPane.INFORMATION_MESSAGE);
            }

        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Debe ingresar un número entero válido en el campo de texto.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void botonEliminarPrimos() {
        ...
        if (eliminado) {
            ...
            JOptionPane.showMessageDialog(this, "No se encontraron números primos en la matriz.");
        }
    }
